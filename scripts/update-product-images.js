...
function getImageUrlForProduct(name, categoryName, index) {
  const lowerName = name.toLowerCase();

  if (lowerName.includes("bóng đèn") || lowerName.includes("led bulb") || lowerName.includes("bán nguyệt")) {
    return "https://images.unsplash.com/photo-1550985616-10810253b84d?auto=format&fit=crop&w=800&q=80";
  }
  if (lowerName.includes("đèn học") || lowerName.includes("bàn") || lowerName.includes("rọi")) {
  ...
  if (lowerName.includes("kìm") || lowerName.includes("tua vít") || lowerName.includes("bộ đồ nghề") || lowerName.includes("thước")) {
  ...
  if (lowerName.includes("công tắc") || lowerName.includes("aptomat") || lowerName.includes("cầu dao")) {
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const newImage = getImageUrlForProduct(product.name, product.category ? product.category.name : "", i);
